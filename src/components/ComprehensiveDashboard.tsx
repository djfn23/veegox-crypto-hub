...
const ComprehensiveDashboard = () => {
  ...
  console.log('ComprehensiveDashboard: State initialized');
  
  // Appeler tous les hooks de manière cohérente
  try {
    console.log('ComprehensiveDashboard: Calling hooks...');
    const { connectedWallet } = useWeb3Wallet();
    console.log('ComprehensiveDashboard: useWeb3Wallet hook called');
    
    const { user, isAuthenticated } = useUnifiedAuth();
    console.log('ComprehensiveDashboard: useUnifiedAuth hook called');
    
    const isMobile = useIsMobile();
    console.log('ComprehensiveDashboard: useIsMobile hook called');
    
    const { isTablet, deviceType, getColumns, getSpacing } = useResponsiveLayout();
    console.log('ComprehensiveDashboard: useResponsiveLayout hook called');
    
    const { data: balanceData, refetch: refetchBalance } = useWalletBalance(connectedWallet?.address || null);
    console.log('ComprehensiveDashboard: useWalletBalance hook called');

    useEffect(() => {
      console.log('ComprehensiveDashboard: useEffect - setting isClient to true');
      setIsClient(true);
    }, []);
    ...
    console.log('ComprehensiveDashboard: About to check isClient:', isClient);

    if (!isClient) {
      console.log('ComprehensiveDashboard: Rendering loading state');
      return (
    ...
    console.log('ComprehensiveDashboard: Rendering main dashboard');
    ...
    console.error('ComprehensiveDashboard: Error caught:', error);
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-white text-center">
          <h2 className="text-xl font-bold mb-2">Erreur de chargement</h2>
          <p className="text-gray-400">Une erreur s'est produite lors du chargement du dashboard.</p>
        </div>
      </div>
    );
  }
